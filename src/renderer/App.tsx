...
function App() {
  ...
  const [searchFilterType, setSearchFilterType] = useState<'all' | 'pending'>(
    'all',
  );
  const [selectedSearchCategoryId, setSelectedSearchCategoryId] = useState<
    string | null
  >(null);
  ...
  const handleCategorySubmit = async () => {
    if (editingCategory) {
      // Editing existing category - use the categoryName field
      if (!categoryName.trim()) return;
      await updateCategory(editingCategory.id, categoryName);
    } else {
      // Creating new category - generate name from year and month
      if (!categoryYear || !categoryMonth) return;
      const generatedName = `${categoryYear}-${categoryMonth}`;
      const newCategory = await createCategory(generatedName);
      setSelectedCategoryId(newCategory.id);
    }
    setCategoryName('');
    setCategoryYear(new Date().getFullYear().toString());
    setCategoryMonth('');
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <header className="sticky top-0 z-[100] flex items-center justify-between border-b border-gray-200 bg-white/90 backdrop-blur-md px-8 py-3">
        <div className="flex items-center gap-8">
          <div className="flex items-center gap-3 text-primary">
            ...
            <h1 className="text-[#111318] text-xl font-bold leading-tight tracking-tight">
              旅游财务核算系统
            </h1>
          </div>
                                ...
                                {result.type === 'customer' &&
                                  result.balance !== undefined && (
                                    <>
                                      ...
                                      <p
                                        className={`text-lg font-bold ${result.balance > 0 ? 'text-red-600' : 'text-gray-900'}`}
                                      >
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {editingCategory ? '编辑分类' : '添加月份'}
            </DialogTitle>
            <DialogDescription>
              {editingCategory ? '修改分类名称' : '选择年份和月份创建新分类'}
            </DialogDescription>
          </DialogHeader>
          <div className="py-4">
            {editingCategory ? (
              ...
              // Create mode: show year and month selectors
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <Label htmlFor="categoryYear">年份</Label>
                    <Input
                      id="categoryYear"
                      type="number"
                      value={categoryYear}
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="categoryMonth">月份</Label>
                    <Select
                      value={categoryMonth}
